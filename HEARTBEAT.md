...
9. 每次 tick 读取 `memory_action`：若请求同步/压缩，交给 LLM 执行语义流程
10. KickCat 不直接读主记忆；仅接收 LLM 结果并通过 apply 落盘
11. 语义压缩上限：任务相关 8KB，非任务相关 4KB（UTF-8 字节）
12. 技能更新时保留本地 KickCat 状态，不清零内部记忆
